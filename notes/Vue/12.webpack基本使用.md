...
==注意==：此时我们打包超过`limit`大小的图片会重新生成在`dist`文件中，但如果我们想要对图片进行管理（命名，导出位置）等，就需要进行以下配置：

![图片处理3](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\图片处理3.jpg)

![大尺寸图片处理](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\大尺寸图片处理.jpg)



# `ES6`语法的处理

将所有我们使用的`ES6`语法的代码打包到`dist`文件夹下为`ES5`语法，成为浏览器能够支持的语法。
...
![vue的配置](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\vue的配置.jpg)

![vue配置2](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\vue配置2.jpg)



# `el`和`template`之间的关系

![el和template](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\el和template.jpg)



# 再次简化上述代码

提取出`template`、`data`、`methods`

![简化](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\简化.jpg)

将定义组件部分再次提取出来

![简化2](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\简化2.jpg)

再次简化

![简化3](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\简化3.jpg)

我们发现，`webpack`并不能识别`.vue`结尾的文件。所以我们需要去配置`vue-loader`和`vue-template-compiler`，前者是对`vue`文件进行一个加载，后者是对`vue`文件进行编译的。
...
- `plugin`是插件，是对`webpack`本省的扩展，是一个扩展器。
...
`BannerPlugin`为打包的文件添加一个版权信息

![插件1](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\插件1.jpg)

`HtmlWebpackPlugin`：

- 自动生成一个`index.html`文件（可以指定模板来生成）
- 将打包的`js`文件自动通过`script`标签插入到`body`中
...
![压缩js](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\压缩js.jpg)

![注意版本号](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\注意版本号.jpg)
...
将`webpack.config.js`分离成1.`base.config.js`用于存储公共的代码配置文件；2.`dev.config.js`用于存储开发时候的代码；3.`prod.config.js`用于存储生产上线时候的配置。也就是说，当我们配置分离完成之后，`webpack.config.js`这个文件就没有用处了。

![分散配置](C:\Users\lenovo\Desktop\2019年11月19日始\notes\Vue\img\分散配置.jpg)
